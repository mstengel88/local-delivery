...
let activeOriginCache: CacheEntry<OriginAddress> | null = null;

const vendorOriginCache = new Map<string, CacheEntry<OriginAddress | null>>();
const distanceMatrixCache = new Map<
  string,
  CacheEntry<(DistancePoint | null)[][] | null>
>();

async function getActiveOriginAddress(): Promise<OriginAddress> {
...
async function getOriginFromVendorLabel(
  ...
  let data: OriginAddress | null = null;
...
function getMaterialFromSku(
  ...
  const rule = rules.find((entry) => entry.prefix === prefix);
...
export async function getQuote(input: QuoteInput): Promise<QuoteResult> {
  ...
  if (totalTrucks === 0) {
    const yardToCustomer = matrix[0]?.[destinationCustomerIndex];
    const customerToYard = matrix[customerOriginIndex]?.[destinationYardIndex];
