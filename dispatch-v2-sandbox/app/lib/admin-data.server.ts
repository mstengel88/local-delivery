...
export async function getShippingMaterialRules(): Promise<ShippingMaterialRuleRow[]> {
  ...
  let data: ShippingMaterialRuleRow[] | null = null;
  ...
  data = (extendedResult.data || null) as ShippingMaterialRuleRow[] | null;
  ...
  if (error?.code === "42703" || /does not exist/i.test(error?.message || "")) {
    const legacy = await supabaseAdmin
      ...
      .select(legacySelect)
      .order("sort_order", { ascending: true });
    data = (legacy.data || null) as ShippingMaterialRuleRow[] | null;
    error = legacy.error;
  }
...
export async function saveShippingMaterialRule(row: ShippingMaterialRuleRow) {
  const payload = {
    ...
    delivery_mode: row.delivery_mode || "bulk",
    capacity_unit: row.capacity_unit || "quantity",
    is_active: row.is_active,
